...
export default function SearchPage({
  ...
  const [filterType, setFilterType] = useState(initialSortBy || "title");
  const [durationRange, setDurationRange] = useState([30]); // Default 30 mins

  const postsPerPage = 15;
  const navigate = useNavigate();

  // Sync state with props if they change (e.g. from URL/Sidebar)
  useEffect(() => {
    if (initialSortBy) setFilterType(initialSortBy);
  }, [initialSortBy]);

  useEffect(() => {
    // If no query and not duration search, skip
    ...
    (async () => {
      try {
        ...
        // Execute search based on selected filter type
        switch (filterType) {
          case "title":
          ...
          case "ingredient":
            data =
              await FoodieService.getPostsContainingIngredient(initialQuery);
            break;
          case "duration":
            // "lt" for less than based on user request implication (duration search)
            // Using duration from slider
            data = await FoodieService.getPostsByDuration(
              durationRange[0],
            );
            break;
          default:
  ...
  }, [initialQuery, filterType, durationRange]); // Re-run when query, filter, or duration changes
