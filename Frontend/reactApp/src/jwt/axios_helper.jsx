    ...
    .then(res => {
         if (messageToLog && messageToLog.trim().length > 0) {
            if(messageToLog === "USER"){
                LogAPI.post("", { logMessage: res.data.type+" logged in" })
                .catch(() => {
                    console.warn("Log operation failed");
                });
            }
            else{
                LogAPI.post("", { logMessage: messageToLog })
                .catch(() => {
                    console.warn("Log operation failed");
                });
            }
...
export const requestJwt = (method, url, data={},messageToLog="") => {
    const userCred = sessionStorage.getItem("userCred").token;
...
export const requestParamJwt = (method, url, data={}, params=0,messageToLog="") => {
    const userCred = sessionStorage.getItem("userCred").token;
    const token = userCred ? userCred.token : "";
    return BackendAPI({
        method,
