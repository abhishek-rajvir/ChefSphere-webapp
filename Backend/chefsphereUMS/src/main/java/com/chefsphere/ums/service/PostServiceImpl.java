...
public class PostServiceImpl implements PostService {
	...
	public void updatePostId(Long postId, PostUpdateDTO dto, HttpServletRequest req) {
		...
		if (dto.getList_of_Steps() != null && !dto.getList_of_Steps().isEmpty()) {
			...
			// remove old steps
			foodCategoryRepo.deleteByRecipe(recipe);
		...
		// ---------- Steps ----------
		if (dto.getSet_of_categorys() != null && !dto.getSet_of_categorys().isEmpty()) {

			// remove old steps
			recipe.removeAllFoodCategory();

			List<FoodCategory> category = dto.getSet_of_categorys().stream().map(s -> {
				FoodCategory fc = mapper.map(s, FoodCategory.class);
				fc.addRecipe(recipe);
				return fc;
			}).toList();
			foodCategoryRepo.saveAll(category);
			recipe.addFoodCategory(category);
		}
	...
	@Override
	@Transactional
	public void updatePostId(Long postId, PostUpdateDTO dto) {

		Post post = postRepo.findByPidAndIsActiveTrue(postId).orElseThrow(()->new InvalidIdException("Post by id"+postId+" does exist"));
	...
	public void deleteById(Long post_id, HttpServletRequest req) {
		...
		Long cid = (creatorRepo.findByUserId_IdAndUserId_IsActiveTrue(Userid)
				.orElseThrow(() -> new InvalidIdException("No creator by user id " + Userid))).getCid();

		if (postRepo.existsByPidAndCreator_CidAndIsActiveTrue(post_id, cid)) {

			Post p = postRepo.findById(post_id).get();
			p.setActive(false);
			postRepo.save(p);
...
//		Set<Recipe> recList = ingredientRepo.findByNameIgnoreCase(ingredient);
		Page<Post> pList = postRepo.findActivePostsByIngredient(ingredient, PageRequest.of(0, 10));
		if (pList.isEmpty()) {
			pList.stream().map(s -> mapper.map(s, PostResponseDTO.class)).toList();
//		if (!recList.isEmpty()) {
//			return postRepo.findByRecipeInAndIsActiveTrue(recList).stream()
//					.map(s -> mapper.map(s, PostResponseDTO.class)).toList();
		}
	...
	public void deleteById(Long post_id) {
		...
		if (postRepo.existsByPidAndIsActiveTrue(post_id)) {
			Post p = postRepo.findById(post_id).get();
			p.setActive(false);
			postRepo.save(p);
		} else {
